...
export const forgetPassword = async (email: string) => {
    return api.post(`/forget-password`, { email }).then((res) => res.data);
};

export const updatePassword = async (user: { email: string; newPassword: string; token: string }) => {
    return api.put(`/update-password`, user).then((res) => res.data);
};

export const githubLogin = async (name: string | undefined) => {
    return api.post(`/github-login`, { name }).then((res) => res.data);
};
...
export const getUsersWithPagination = async ({ page, limit }: { page: number; limit: number }) => {
    return api.get(`/users/pagination/${page}/${limit}`).then((res) => res.data);
};
...
export const updateCurrentUser = async (user: {
    ...
    return api.put(`/update-user/${user.id}`, user).then((res) => res.data);
};
